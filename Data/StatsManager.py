...
def print_contextual_graphs(rewards_per_experiment_ucb, rewards_per_experiment_ts, clairvoyant):
    rewards_ts = []
    rewards_ucb = []
    for exp_rewards in rewards_per_experiment_ts:
        rewards_ts.append(list(chain(*exp_rewards)))
    for exp_rewards in rewards_per_experiment_ucb:
        rewards_ucb.append(list(chain(*exp_rewards)))
    for customer_class in range(3):
        printReward(rewards_ucb, clairvoyant[customer_class], "UCB: customer class "+str(customer_class))
        printReward(rewards_ts, clairvoyant[customer_class], "TS: customer class "+str(customer_class))
        printRegret(np.asarray(rewards_ucb), np.asarray(rewards_ts), clairvoyant[customer_class], "UCB and TS regrets")
