...
class SocialInfluence():
    ...
    def ecommerce_user_simulation(self, number_of_products, initial_active_nodes, price_configuration, price_campaign,
                                  customer_class, step_4, step_5, customers):
        # assign a value for the lambda coefficient for the second secondary product
        lambda_coefficient = 0.5
        # store probablity_matrix of the current customer class
        prob_matrix = 0
        if not step_5:
            prob_matrix = np.copy(customers[customer_class].social_influence_transition_probability_matrix)
        else:
            prob_matrix = (customers[0].social_influence_transition_probability_matrix + customers[
                1].social_influence_transition_probability_matrix + customers[
                               2].social_influence_transition_probability_matrix) / 3
        ...
        if customers[customer_class].reservation_prices[int(np.where(active_nodes == 1)[0])] < price_configuration[
            int(np.where(active_nodes == 1)[0])]:
            return history
        # update transition probability of the new active nodes to zero value so that it is not possible to reach again the same node (product)
        ...
        while (t < n_steps_max and np.sum(newly_active_nodes) > 0):
            ...
            transition_probabilities_from_the_active_node = (prob_matrix.T * active_nodes).T
            # select from the probability rows just the ones related to the active node
            p_row = transition_probabilities_from_the_active_node[np.where(active_nodes == 1)][0]
            ...
            # assign false to all the activated edges array to keep track of the one that will be selected (clicked secondary product)
            activated_edges = [False for _ in range(5)]

            # parte modificata da Alice

            # one or two secondary products are chosen
            num_prod_clicked = np.random.randint(1, 3)
            # random choice of the index of the secondary product selected by the user (select one, but maybe could be two CHECK THIS THING)
            ...
            for i in range(5):
                # if the chosen secondary product is found, let the costumer actualy buy it, by updated the values related to the units sold and the revenue
                if (activated_edges[i] == True and customers[customer_class].reservation_prices[i] >=
                    ...
                    # update the amound of unites of product purchased by the class of user
                    if not step_4:
                        customers[customer_class].units_purchased_for_each_product[i] += units_purchased
                        ...
                        price_campaign.aggregate_units_sold_per_product[i] += units_purchased
                        # if the purchase cap is not reached, then activate the node to continue the simulation
                        if ((customers[0].max_number_of_purchases + customers[1].max_number_of_purchases + customers[
                            2].max_number_of_purchases) >= sum(price_campaign.aggregate_units_sold_per_product)):
                            # assign 1 to the new active nodes
                            newly_active_nodes[i] = 1
            # update transition probability of the new active nodes to zero value so that it is not possible to reach again the same node (product)
            for i in range(prob_matrix.shape[1]):
        ...
        # a function to evauate the conversion rate of each customer class, by computing thenumber of occurences in global history (n. of sales) over the number of customers belonging to the class

    def evaluate_conversion_rate(customer_class, price_campaign, price_configuration, customers):
        ...
        if price_campaign.global_history[customer_class] is not None:
            ...
            # a the end of the loop, evaluate the number of no-sales, by subtracting the number of sales from the total number of customers
            price_campaign.no_sales[customer_class] = customers[customer_class].number_of_customers - \
                                                      price_campaign.sales[customer_class]
            # the conversione rate is equal to the number of sales over the number of customer of the current class. the conversion rate is relative to the whole price campaing (price configuration)
    ...
    def evaluate_aggregate_conversion_rate(self, price_campaign, price_configuration, customers):
        total_sales = 0
        # check if the global history is not null (just to be sure)
        for customer_class in range(3):
            ...
            total_conversion = total_sales / (
                    customers[0].number_of_customers + customers[1].number_of_customers + customers[
                2].number_of_customers)
            # the conversione rate is equal to the number of sales over the number of customer of the current class. the conversion rate is relative to the whole price campaing (price configuration)
            # store the value of the conversion rate
    ...
    def evaluate_profit(self, customer_class, price_campaign, price_configuration, customers):
    ...
    def simulation(self, number_of_products, price_configuration, customer_class, price_campaign, aggregate_conversion,
        ...
        for j in range(n):
            ...
            if not step_4:
                ...
                alpha_matrix = [customers[0].alpha_probabilities[1:5], customers[1].alpha_probabilities[1:5],
                                customers[2].alpha_probabilities[1:5]]

                alpha_mean = np.mean(alpha_matrix, axis=0)
                initial_active_nodes[np.random.choice(np.arange(0, 5), alpha_mean)] = 1
            ...
            result = self.ecommerce_user_simulation(number_of_products, initial_active_nodes, price_configuration,
                                               price_campaign, customer_class, step_4, step_5, customers)
            ...
            price_campaign.marginal_profit[customer_class] = self.evaluate_profit(customers[customer_class], price_campaign,
                                                                             price_configuration, customers)
            if not aggregate_conversion:
                print(
                    'For customer class {0} and configuration {1}, the conversion rate is {2} and the profit is {3}'.format(
                        customer_class, price_campaign.id, conversion_rate,
                        price_campaign.marginal_profit[customer_class]))

    def run_social_influence_simulation(self, number_of_products, price_configuration, customer_class, price_campaign,
                                        aggregate_conversion, step_4, step_5, customers):
        if customer_class == 3 and aggregate_conversion:
            ...
            self.simulation(number_of_products, price_configuration, customer_class, price_campaign, aggregate_conversion,
                       step_4, step_5, customers)
