...
class SocialInfluence():
    ...
    def __init__(self):
        ...
        def ecommerce_user_simulation(number_of_products, initial_active_nodes, price_configuration, price_campaign,
                                      customer_class):
            ...
            while (t < n_steps_max and np.sum(newly_active_nodes) > 0):
                ...
                indx = random.choices(np.arange(0, 5), p_row, k=1)
    ...
    def evaluate_conversion_rate(customer_class, price_campaign, price_configuration):
        ...
        def evaluate_aggregate_conversion_rate(price_campaign, price_configuration):
            total_sales = 0
            # check if the global history is not null (just to be sure)
    ...
    def evaluate_profit(customer_class, price_campaign, price_configuration):
    ...
    def simulation(number_of_products, price_configuration, customer_class, price_campaign, aggregate_conversion):
        ...
        for j in range(n):
            ...
            result = ecommerce_user_simulation(number_of_products, initial_active_nodes, price_configuration,
        ...
        conversion_rate = evaluate_conversion_rate(customer_class, price_campaign, price_configuration)
        # evaluate the marginal profit of the current price campaign (price configuration) and customer class
        price_campaign.marginal_profit[customer_class] = evaluate_profit(customers[customer_class], price_campaign,
                                                                         price_configuration)
    ...
    def run_social_influence_simulation(number_of_products, price_configuration, customer_class, price_campaign,
                                        aggregate_conversion):
        if customer_class == 3 and aggregate_conversion:
            for customer_class in range(3):
                simulation(number_of_products, price_configuration, customer_class, price_campaign,
                           aggregate_conversion)
            aggregate_conversion_rate = evaluate_aggregate_conversion_rate(price_campaign, price_configuration)
            print('For configuration {0}, the aggregate conversion rate is {1}.'.format(price_campaign.id,
                                                                                        aggregate_conversion_rate))
        else:
            simulation(number_of_products, price_configuration, customer_class, price_campaign, aggregate_conversion)