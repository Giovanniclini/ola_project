            ...
            for i in range(6):
                ...
                if step == 3:
                    if campaigns[idx].sales[customer_class] == 0:
                        social.run_social_influence_simulation(number_of_products, campaigns[idx].configuration, customer_class, campaigns[idx], customers, step)
                # assign conversion rate evaluated in social influence
                p[i] = campaigns[idx].conversion_rate[customer_class]

                ts_p = np.copy(campaigns[idx].aggregate_conversion_rate_per_product)
